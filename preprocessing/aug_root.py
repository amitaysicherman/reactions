...
def preprocess(multi_process_func, save_dir, data, set_name, augmentation=1, character=False, processes=-1):
    """
...
if __name__ == '__main__':
    ...
    for i, data_set in enumerate(datasets):
        ...
        final_ec = []
        data = []
        assert len(reactant_smarts_list) == len(product_smarts_list)
        if ec_list:
            assert len(reactant_smarts_list) == len(ec_list)
        skips_count_dict = {'empty_p': 0, 'empty_r': 0, 'invalid_r': 0, 'small_r': 0, 'invalid_p': 0, 'small_p': 0,
                            'error_mapping_p': 0}
        for i in range(len(reactant_smarts_list)):
            product = product_smarts_list[i]
            reactant = reactant_smarts_list[i]
            pro_mol = Chem.MolFromSmiles(product)
            rea_mol = Chem.MolFromSmiles(reactant)
            if "" == product:
                skips_count_dict["empty_p"] += 1
                continue
            if "" == reactant:
                skips_count_dict["empty_r"] += 1
                continue
            if rea_mol is None:
                skips_count_dict["invalid_r"] += 1
                continue
            if len(rea_mol.GetAtoms()) < 5:
                skips_count_dict["small_r"] += 1
                continue
            if pro_mol is None:
                skips_count_dict["invalid_p"] += 1
                continue
            if len(pro_mol.GetAtoms()) == 1:
                skips_count_dict["small_p"] += 1
                continue
            if not all([a.HasProp('molAtomMapNumber') for a in pro_mol.GetAtoms()]):
                skips_count_dict["error_mapping_p"] += 1
                continue
            if ec_list:
                final_ec.append(ec_list[i])
            data.append({
                "reactant": reactant,
                "product": product,
